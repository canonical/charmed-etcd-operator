...
async def test_build_and_deploy_with_tls(charm: str, ops_test: OpsTest) -> None:
    ...
    await ops_test.model.deploy(
        VAULT_NAME,
        channel="1.18/edge",
        config={"pki_ca_common_name": DOMAIN_NAME, "pki_allow_subdomains": True},
    )
    await ops_test.model.integrate(f"{VAULT_NAME}:tls-certificates-pki", TLS_NAME)

    # Deploy the charm and wait for active/idle status
    logger.info("Deploying the charm")
    await ops_test.model.deploy(charm, num_units=NUM_UNITS, config={"common_name": DOMAIN_NAME})

    await wait_until(
        ops_test,
        apps=[APP_NAME, TLS_NAME, VAULT_NAME],
        apps_full_statuses={
            APP_NAME: {
                "active": [],
            },
            TLS_NAME: {"active": []},
            VAULT_NAME: {
                "blocked": ["Please initialize Vault or integrate with an auto-unseal provider"],
            },
        },
    )
