...
def test_ca_rotation_by_config_change(juju_vm_model: Juju) -> None:
    ...
    current_peer_ca = get_certificate_from_unit(
        juju_vm_model, leader_unit, cert_type=TLSType.PEER, is_ca=True
    )
    assert current_peer_ca, "Failed to get the current peer CA certificate"

    current_client_ca = get_certificate_from_unit(
        juju_vm_model, leader_unit, cert_type=TLSType.CLIENT, is_ca=True
    )
    ...
    current_peer_certificate = get_certificate_from_unit(
        juju_vm_model, leader_unit, cert_type=TLSType.PEER, is_ca=False
    )
    ...
    current_client_certificate = get_certificate_from_unit(
        juju_vm_model, leader_unit, cert_type=TLSType.CLIENT, is_ca=False
    )
...
def test_ca_rotation_by_expiration(juju_vm_model: Juju) -> None:
    ...
    """
    _prepare_units_for_ca_expiration_test(juju_vm_model)
