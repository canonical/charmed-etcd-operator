...
async def resume_refresh(
    ops_test: OpsTest, 
    app_name: str,
    check_health_of_refreshed_units: bool = True
) -> Dict:
...
async def verify_cluster_data_integrity(
    ...
    try:
        ...
        if len(output_lines) >= 2 and output_lines[1] == test_value:
            ...
            logger.error(f"Data integrity check failed. Expected: {test_value}, Got: {output_lines}")
            return False
...
class RefreshTestContext:
    ...
    async def __aenter__(self):
        ...
        if self.verify_data_integrity:
            await verify_cluster_data_integrity(
                self.ops_test, self.app_name, 
                self.test_data_key, self.test_data_value
            )
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        ...
        for unit_name in self.initial_snap_revisions:
            if (unit_name in final_snap_revisions and 
                self.initial_snap_revisions[unit_name] != final_snap_revisions[unit_name]):
