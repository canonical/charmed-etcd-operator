        ...
        async with RefreshTestContext(ops_test, APP_NAME, verify_data_integrity=True):
            ...
            try:
                ...
                try:
                    ...
                    success_rate = (
                        sum(workload_results) / len(workload_results) if workload_results else 0
                    )
                    ...
                    assert success_rate >= 0.8, (
                        f"Workload success rate too low: {success_rate:.2%}"
                    )
    ...
    async def test_refresh_with_different_pause_strategies(self, ops_test: OpsTest, etcd_cluster):
        ...
        for strategy in pause_strategies:
            ...
            async with RefreshTestContext(ops_test, APP_NAME):
                ...
                await set_refresh_config(
                    ops_test, APP_NAME, {"pause_after_unit_refresh": strategy}
                )
    ...
    async def test_refresh_with_resource_constraints(self, ops_test: OpsTest, etcd_cluster):
        ...
        async with RefreshTestContext(ops_test, APP_NAME):
            ...
            health_task = asyncio.create_task(
                check_cluster_health_during_refresh(ops_test, APP_NAME, check_duration=120)
            )
...
class TestRefreshPerformance:
    ...
    async def test_refresh_timing_metrics(self, ops_test: OpsTest, etcd_cluster):
        ...
        async with RefreshTestContext(ops_test, APP_NAME):
            ...
            assert revision_duration < 30, (
                f"Snap revision query too slow: {revision_duration:.2f}s"
            )
