    ...
    async def test_pre_refresh_check_with_healthy_cluster(self, ops_test: OpsTest, etcd_cluster):
        ...
        async with RefreshTestContext(ops_test, APP_NAME) as ctx:
            ...
            integrity_ok = await verify_cluster_data_integrity(
                ops_test, APP_NAME,
                test_key="health-test-key",
                test_value="health-test-value"
            )
    ...
    async def test_pre_refresh_check_with_different_configs(self, ops_test: OpsTest, etcd_cluster):
        ...
        for config in configs:
            ...
            async with RefreshTestContext(ops_test, APP_NAME) as ctx:
                ...
                assert current_config["pause_after_unit_refresh"] == config["pause_after_unit_refresh"]
...
class TestRefreshActionValidation:
    ...
    async def test_action_parameter_validation(self, ops_test: OpsTest, etcd_cluster):
        ...
        async with RefreshTestContext(ops_test, APP_NAME) as ctx:
            ...
            try:
                action = await leader_unit.run_action("force-refresh-start", **{
...
class TestRefreshActionIntegration:
    ...
    async def test_actions_with_snap_revision_tracking(self, ops_test: OpsTest, etcd_cluster):
        ...
        async with RefreshTestContext(ops_test, APP_NAME) as ctx:
            ...
            after_check_revisions = await get_all_snap_revisions(ops_test, APP_NAME, DEFAULT_SNAP_NAME)
            ...
            assert initial_revisions == after_check_revisions, "Snap revisions should not change during pre-refresh check"
            ...
            assert len(final_revisions) == len(initial_revisions), "Should track revisions for all units"
