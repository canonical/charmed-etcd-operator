...
def test_build_and_deploy(charm: str, juju_vm_model: Juju) -> None:
    ...
    juju_vm_model.cli("create-storage-pool", "etcd-pool", "lxd")
    ...
    juju_vm_model.deploy(charm, num_units=NUM_UNITS, storage=storage)
    ...
    wait_until_apps_active_and_agents_idle(juju_vm_model, APP_NAME)
...
def test_attach_storage_after_scale_down(juju_vm_model: Juju) -> None:
    ...
    unit = list(juju_vm_model.status().get_units(app))[-1]
    data_storage_id = get_storage_id(juju_vm_model, unit, "data")
    archive_storage_id = get_storage_id(juju_vm_model, unit, "archive")
    log_storage_id = get_storage_id(juju_vm_model, unit, "log")
    ...
    juju_vm_model.add_unit(
        app, attach_storage=[data_storage_id, archive_storage_id, log_storage_id]
    )

    new_unit = list(juju_vm_model.status().get_units(app))[-1]
    juju_vm_model.wait(
        lambda status: are_apps_active_and_agents_idle(
    ...
    unit_endpoint = get_unit_endpoint(juju_vm_model, unit_name=new_unit, app_name=app)
...
def test_attach_storage_after_scale_to_zero(juju_vm_model: Juju) -> None:
    ...
    secret = get_secret_by_label(juju_vm_model, label=f"{PEER_RELATION}.{app}.app")
    password = secret.get(f"{INTERNAL_USER}-password")
    initial_endpoints = get_cluster_endpoints(juju_vm_model, app)
    ...
    for storage_id in storage_ids:
        juju_vm_model.add_unit(app, attach_storage=storage_id)
...
def test_attach_storage_after_removing_application(charm: str, juju_vm_model: Juju) -> None:
    ...
    unit = list(juju_vm_model.status().get_units(app))[0]
    storage_id = get_storage_id(juju_vm_model, unit, "data")
    ...
    juju_vm_model.deploy(charm, attach_storage=storage_id)
    ...
    new_unit = list(juju_vm_model.status().get_units(app))[-1]
    unit_endpoint = get_unit_endpoint(juju_vm_model, unit_name=new_unit, app_name=APP_NAME)
    assert (
        put_key(
            unit_endpoint,
            user=INTERNAL_USER,
    ...
    # scale up
    juju_vm_model.add_unit(APP_NAME, num_units=2)
    juju_vm_model.wait(
        lambda status: are_apps_active_and_agents_idle(
