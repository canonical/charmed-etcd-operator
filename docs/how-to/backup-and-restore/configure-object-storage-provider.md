...
Brief context about how connecting to cloud services requires an intermediate integrator charm for authentication/credentials management.
...
```{terminal}
:input: juju deploy s3-integrator
```
...
Finally, configure the connection parameters of your S3 storage to `s3-integrator`:
```{terminal}
:input: juju config s3-integrator \
...
```{terminal}
:input: juju deploy azure-storage-integrator --channel latest/edge
```
...
```{terminal}
:input: juju config azure-storage-integrator storage-account=<Azure_storage_account> container=<Azure_storage_container>
```
...
```{terminal}
:input: juju add-secret mysecret secret-key=<Azure_storage_key>
secret:d183rs296n7svmjrivp0

:input: juju grant-secret mysecret azure-storage-integrator

:input: juju config azure-storage-integrator credentials=secret:d183rs296n7svmjrivp0 
```
...
```{terminal}
:input: juju integrator <storage-integrator-charm> charmed-etcd
```

You can see a confirmation message in the log:
```{terminal}
...
```{terminal}
:input: juju config <storage-integrator-charm> <option>=<value>
```